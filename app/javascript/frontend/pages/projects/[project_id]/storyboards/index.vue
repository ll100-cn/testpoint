...
<PageHeader>
  ...
  <template #actions v-if="storyboard_id">
  </template>
</PageHeader>

<Nav v-model:model-value="storyboard_id">
  <NavList preset="tabs">
    <NavItem v-for="storyboard in storyboards" :value="storyboard.id.toString()" @click.prevent="changeStoryboard(storyboard)">{{ storyboard.title }}</NavItem>
...
<Card class="min-h-96 flex-1 rounded-ss-none" v-if="storyboard_id">
...
const storyboard_id = ref(query.storyboard_id)
const storyboard = ref(null as Storyboard | null | undefined)
...
if (storyboards.value.length > 0 && storyboard_id.value == null) {
  storyboard_id.value = storyboards.value[0].id.toString()
}

const requirements = ref([] as Requirement[])
if (storyboard_id.value) {
  storyboard.value = storyboards.value.find((s) => s.id.toString() === storyboard_id.value)

  requirements.value = await new q.project.RequirementReq.List().setup(proxy, (req) => {
    req.interpolations.project_id = project_id
    req.interpolations.storyboard_id = storyboard_id.value
  }).perform()
}
